...
function cd_qotd_quote() {
	...
	echo '<div class="cd_quote_container">';
	echo '<'.$qotd_type.' cite="'.stripslashes($result[0]->cite).'">';
	echo '<span class="cd_qotd_quote">'.stripslashes($result[0]->quote).'</span>';
	echo '</'.$qotd_type.'>';
	echo '<div class="cd_qotd_author" title="'.stripslashes($result[0]->cite).'">~'.stripslashes($result[0]->author).'</div>';
	echo '</div>';
}
